...
//TODO ES6: This will most likely have to be changed so the users of this library can 'provide' their own app store implementation
...
ApiClient.prototype.getUrl = function(name, params) {
  ...
  if (
    lowered.indexOf('/emby') === -1 &&
    lowered.indexOf('/mediabrowser') === -1
  ) {
    url += '/emby';
  }
...
function switchConnectionMode(instance, connectionMode) {
  ...
  if (
    MediaBrowser.ServerInfo.getServerAddress(
      currentServerInfo,
      newConnectionMode
    )
  ) {
...
function tryReconnectInternal(
  ...
  var url = MediaBrowser.ServerInfo.getServerAddress(
    instance.serverInfo(),
    connectionMode
  );
  ...
  var timeout =
    connectionMode === MediaBrowser.ConnectionMode.Local ? 7000 : 15000;
  ...
  return new Promise(function(resolve, reject) {
    setTimeout(function() {
      tryReconnectInternal(
        ...
        resolve,
        reject,
        instance.serverInfo().LastConnectionMode,
        0
      );
    }, 300);
  });
...
ApiClient.prototype.getCurrentUser = function(enableCache) {
  ...
  var serverPromise = this.getUser(userId).then(
    function(user) {
      appStorage.setItem(
        'user-' + user.Id + '-' + user.ServerId,
        JSON.stringify(user)
      );
...
function onWebSocketMessageInternal(instance, msg) {
  if (msg.MessageType === 'UserDeleted') {
    ...
    msg.MessageType === 'UserPolicyUpdated'
  ) {
    var user = msg.Data;
...
ApiClient.prototype.updateServerInfo = function(server, connectionMode) {
  ...
  var serverUrl = MediaBrowser.ServerInfo.getServerAddress(
    server,
    connectionMode
  );
...
ApiClient.prototype.performEpisodeOrganization = function(id, options) {
  var url = this.getUrl(
    'Library/FileOrganizations/' + id + '/Episode/Organize'
  );
 ...
 * @param {String} name
 */
ApiClient.prototype.addVirtualFolder = function(
  name,
  type,
  refreshLibrary,
  libraryOptions
 ...
 * @param {String} name
 */
ApiClient.prototype.renameVirtualFolder = function(
  name,
  newName,
  refreshLibrary
) {
 ...
 * @param {String} name
 */
ApiClient.prototype.removeMediaPath = function(
  virtualFolderName,
  mediaPath,
  refreshLibrary
) {
...
ApiClient.prototype.updateItemImageIndex = function(
  itemId,
  imageType,
  imageIndex,
  newIndex
...
function normalizeImageOptions(instance, options) {
  ...
  options.quality =
    options.quality || instance.getDefaultImageQuality(options.type);
 ...
 */
ApiClient.prototype.updateUserPassword = function(
  userId,
  currentPassword,
  newPassword
) {
...
ApiClient.prototype.getDateParamValue = function(date) {
  ...
  return (
    '' +
    d.getFullYear() +
    formatDigit(d.getMonth() + 1) +
    formatDigit(d.getDate()) +
    formatDigit(d.getHours()) +
    formatDigit(d.getMinutes()) +
    formatDigit(d.getSeconds())
  );
};
 ...
 */
ApiClient.prototype.updateFavoriteStatus = function(
  userId,
  itemId,
  isFavorite
) {
...
ApiClient.prototype.reportPlaybackProgress = function(options) {
  ...
  if ((options.EventName || 'timeupdate') === 'timeupdate') {
    ...
    if (msSinceLastReport <= 10000) {
      ...
      var expectedReportTicks =
        msSinceLastReport * 10000 + (this.lastPlaybackProgressReportTicks || 0);
...
ApiClient.prototype.sendPlayStateCommand = function(
  sessionId,
  command,
  options
) {
...
ApiClient.prototype.getPackageReviews = function(
  packageId,
  minRating,
  maxRating,
  limit
...
ApiClient.prototype.getEndpointInfo = function() {
  ...
  var instance = this;
  return this.getJSON(this.getUrl('System/Endpoint')).then(function(
    endPointInfo
  ) {
