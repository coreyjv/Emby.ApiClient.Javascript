//TODO ES6: I believe this is importing the regular API client but need to confirm
...
function getLocalView(instance, serverId, userId) {
  return instance.getLocalFolders(serverId, userId).then(({ length }) => {
    let localView = null;
...
class ApiClientEx {
  ...
  getPlaybackInfo(itemId, options, deviceProfile) {
    const onFailure = () =>
      ApiClient.prototype.getPlaybackInfo.call(
        instance,
        itemId,
        options,
        deviceProfile
    ...
    if (isLocalId(itemId)) {
      return localassetmanager
        .getLocalItem(this.serverId(), stripLocalPrefix(itemId))
        .then(({ Item }) => {
          // TODO: This was already done during the sync process, right? If so, remove it
          const mediaSources = Item.MediaSources.map(m => {
    ...
    var instance = this;
    return localassetmanager
      .getLocalItem(this.serverId(), itemId)
      .then(item => {
        if (item) {
          ...
          return localassetmanager.fileExists(item.LocalPath).then(exists => {
            if (exists) {
              const res = {
                MediaSources: mediaSources
              };
  ...
  getItem(userId, itemId) {
    ...
    if (isLocalViewId(itemId)) {
      ...
      if (serverInfo) {
        return this.getLocalFolders(serverInfo.Id, userId).then(items => {
          const views = items.filter(({ Id }) => Id === itemId);
            ...
            if (
              ...
              libraryItem.UserData.PlayedPercentage = Math.min(
                libraryItem.RunTimeTicks
                  ? 100 *
                    ((options.PositionTicks || 0) / libraryItem.RunTimeTicks)
                  : 0,
                100
              );
  ...
  getItemDownloadUrl(itemId) {
    if (isLocalId(itemId)) {
      ...
      if (serverInfo) {
        return localassetmanager
          .getLocalItem(serverInfo.Id, stripLocalPrefix(itemId))
          .then(({ LocalPath }) => Promise.resolve(LocalPath));
      }
    }
