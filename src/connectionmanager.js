...
//TODO ES6: This will most likely have to be changed so the users of this library can 'provide' their own server discovery implementation
...
var ConnectionManager = function(
  ...
  function addAuthenticationInfoFromConnect(
    ...
    url = getEmbyServerUrl(
      url,
      'Connect/Exchange?format=json&ConnectUserId=' + credentials.ConnectUserId
    );
  ...
  self.getAvailableServers = function() {
    ...
    return Promise.all([getConnectServers(credentials), findServers()]).then(
      function(responses) {
  ...
  self.connectToServer = function(server, options) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (tests.indexOf(ConnectionMode.Manual) === -1) {
        tests.push(ConnectionMode.Manual);
      }
      if (tests.indexOf(ConnectionMode.Local) === -1) {
        tests.push(ConnectionMode.Local);
      }
      if (tests.indexOf(ConnectionMode.Remote) === -1) {
        tests.push(ConnectionMode.Remote);
      }
  ...
  function testNextConnectionMode(tests, index, server, options, resolve) {
    ...
    console.log(
      'testing connection mode ' + mode + ' with server ' + server.Name
    );

    tryConnect(address, timeout).then(
      function(result) {
  ...
  self.getRegistrationInfo = function(feature, apiClient, options) {
    ...
    return updateDevicePromise.then(function() {
      return apiClient.getCurrentUser().then(function(user) {
        ...
        if (
          user.Id.toLowerCase() === '81f53802ea0247ad80618f55d9b4ec3c' &&
          params.serverId.toLowerCase() === '21585256623b4beeb26d5d3b09dec0ac'
        ) {
          return Promise.reject();
        }
