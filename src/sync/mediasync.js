    ...
    return localassetmanager
      ...
      .then(function(items) {
        ...
        var progressItems = items.filter(function(item) {
          return (
            item.SyncStatus === 'transferring' || item.SyncStatus === 'queued'
          );
        });
        ...
        return p.then(function() {
          console.log(
            '[mediasync] Exit processDownloadStatus. Items reported: ' +
              cnt.toString()
          );
          return Promise.resolve();
        });
    ...
    .then(function(actions) {
      ...
      return apiClient.reportOfflineActions(actions).then(
        ...
        function(err) {
          ...
          // never gets resolved
          console.error(
            '[mediasync] error on apiClient.reportOfflineActions: ' +
              err.toString()
          );
...
function syncData(apiClient, serverInfo, syncUserItemAccess) {
  ...
  return localassetmanager.getServerItems(serverInfo.Id).then(function(items) {
    var completedItems = items.filter(function(item) {
      return (
        item && (item.SyncStatus === 'synced' || item.SyncStatus === 'error')
      );
    });
  ...
  return localassetmanager
    ...
    .then(function(existingItem) {
      var onDownloadParentItemsDone = function(localItem) {
        return downloadMedia(apiClient, jobItem, localItem, options).then(
          function() {
...
function downloadImage(
  ...
  return localassetmanager.hasImage(serverId, itemId, imageType, index).then(
    function(hasImage) {
      if (hasImage) {
        console.log(
          '[mediasync] downloadImage - skip existing: ' +
            itemId +
            ' ' +
            imageType +
            '_' +
...
function getItemSubtitle(file, apiClient, jobItem, localItem, mediaSource) {
  ...
  var fileName = localassetmanager.getSubtitleSaveFileName(
    localItem,
          ...
          p = p.then(function() {
            return localassetmanager
              ...
              .then(function(exists) {
                if (!exists) {
                  return localassetmanager.removeLocalItem(completedItem).then(
                    function() {
                      return Promise.resolve();
                    },
                    function() {
...
export default function() {
  ...
  self.sync = function(apiClient, serverInfo, options) {
    ...
    return checkLocalFileExistence(apiClient, serverInfo, options).then(
      function() {
        return processDownloadStatus(apiClient, serverInfo, options).then(
          function() {
            return localassetmanager
              .getDownloadItemCount()
              .then(function(downloadCount) {
                if (
                ...
                return reportOfflineActions(apiClient, serverInfo).then(
                  function() {
                    ...
                    return getNewMedia(
                      ...
                      return syncData(apiClient, serverInfo, false).then(
                        function() {
                          console.log(
                            '[mediasync]************************************* Exit sync'
                          );
